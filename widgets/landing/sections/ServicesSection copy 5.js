...
export default function ServicesSection() {
  ...
  return (
    <section id="services" className="relative isolate py-20 sm:py-28">
      {/* ambient brand glow (kept subtle, industrial) */}
      <div
        aria-hidden
      ...
      <div className="mx-auto max-w-7xl px-6">
        ...
        {/* INDUSTRIAL KPI STRIP */}
        <KpiStrip className="mt-8" />

        {/* Controls: brand-styled segmented toggle */}
        ...
        {/* CTA RAIL (replaces single bottom button) */}
        <CtaRail className="mt-12" />
      </div>
    </section>
  );
}
...
/* ----------------------- KPI STRIP (industrial) ----------------------- */
function KpiStrip({ className = "" }) {
  const items = [
    {
      label: "Lead time",
      value: "Lab → Pilot",
      meta: "Coordinated iterations",
    },
    {
      label: "Compliance",
      value: "EU/DZ Ready",
      meta: "INCI • Claims • Files",
    },
    {
      label: "Stability plan",
      value: "Validated",
      meta: "Accel/real‑time • Pack fit",
    },
    { label: "Sourcing", value: "Traceable", meta: "Local botanicals" },
  ];

  return (
    <div
      className={cx(
        "grid gap-0 overflow-hidden rounded-xl border border-[var(--tile-border)] bg-[var(--surface-1)] shadow-[var(--tile-shadow)] sm:grid-cols-2 lg:grid-cols-4",
        className
      )}
    >
      {items.map((k, i) => (
        <div
          ...
          className={cx(
            ...
            i % 2 === 0 ? "bg-[var(--surface-0)]" : "bg-[var(--tile-bg)]",
            "border-b border-[var(--tile-border)] sm:border-b-0",
            "lg:[&:not(:last-child)]:border-r lg:border-[var(--tile-border)]"
          )}
        >
          ...
          <p className="whitespace-nowrap text-[12.5px] text-[var(--text-secondary)]">
            {k.meta}
          </p>
        </div>
      ))}
...
function SegmentedToggle({ value, onChange }) {
  ...
  const active = "bg-[var(--cta-700)] text-white shadow-sm hover:brightness-95";
  ...
  return (
    ...
    >
      <button
        ...
        className={cx(base, value === "compare" ? active : inactive)}
      >
        Comparison
      </button>
      <button
        ...
        className={cx(base, value === "cards" ? active : inactive)}
      >
        Services
      </button>
    </div>
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-[var(--tile-border)] shadow-[var(--tile-shadow)]">
      <div className="overflow-x-auto">
        <table
          className="
            w-full border-collapse leading-[1.45]
      ...
      <div className="border-t border-[var(--tile-border)] bg-[var(--surface-1)] px-4 py-3 text-[12.5px] text-[var(--text-secondary)]">
        Scope: cosmetics • nutraceuticals • biopesticides • animal nutrition •
        agri‑food.
      </div>
...
/* --------------------------- CTA RAIL --------------------------- */
function CtaRail({ className = "" }) {
  return (
    <div
      className={cx(
        "rounded-2xl border border-[var(--tile-border)] bg-[var(--surface-1)] p-4 shadow-[var(--tile-shadow)]",
        className
      )}
    >
      <div className="flex flex-col items-center justify-between gap-3 sm:flex-row">
        <div className="text-center sm:text-left">
          ...
          <h4 className="text-[16px] font-semibold text-[var(--text-primary)]">
            Tell us your constraints — we’ll scope, test and transfer.
          </h4>
        </div>

        <div className="flex flex-wrap items-center gap-2">
          {/* Primary action: book a technical call */}
          <Button
            variant="primary"
            size="md"
            asLink
            href="#contact"
            aria-label="Book a technical call"
          >
            Book a technical call
          </Button>

          {/* Secondary action: request scope/quote */}
          <Link
            href="#contact"
            className="
              inline-flex items-center rounded-lg border px-3.5 py-2 text-sm font-semibold
              border-[var(--tile-border)] text-[var(--text-primary)] hover:bg-[var(--surface-0)]
            "
            aria-label="Request scope and quote"
          >
            Request scope & quote
          </Link>

          {/* Tertiary: read regulatory scope */}
          <Link
            href="/expertise"
            className="inline-flex items-center rounded-lg px-3 py-2 text-sm font-medium text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
            aria-label="Read regulatory scope"
          >
            Regulatory scope →
          </Link>
        </div>
