...
// Lightweight, no dependencies (icons are inline SVGs), a11y-friendly
...
function ServiceTile({ title, line, icon: Icon, image, bullets = [], cta }) {
  return (
    ...
    shadow-[0_1px_2px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgba(20,184,166,0.12)]
    transition
      "
    >
      ...
      <div className="relative h-32">
        {image ? (
          ...
          <div
            ...
            style={{
              background:
                "radial-gradient(600px 180px at 30% 30%, rgba(16,185,129,0.2), transparent), radial-gradient(600px 180px at 70% 70%, rgba(34,211,238,0.18), transparent)",
            }}
          />
        )}
      </div>
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-[40px,1fr] items-center gap-3">
          <span className="grid h-10 w-10 place-items-center rounded-xl bg-teal-50 text-teal-700 dark:bg-teal-500/15 dark:text-teal-200 border border-teal-200/60 dark:border-teal-400/20">
            <Icon />
          </span>
        ...
        <div className="mt-5 flex flex-wrap items-center gap-3">
          <a
            href="#contact"
            className="inline-flex items-center rounded-lg bg-teal-500 px-3.5 py-2 text-xs font-semibold text-white hover:bg-teal-600 transition"
          >
            {cta}
          </a>
          ...
          <details className="group/details relative w-auto">
            <summary
              className="list-none cursor-pointer select-none inline-flex items-center gap-2
               text-xs font-semibold text-slate-700 dark:text-slate-200 hover:underline
               focus:outline-none focus-visible:ring-4 focus-visible:ring-teal-300/35 rounded"
            >
              <span>Read more</span>
              <svg
                className="h-3.5 w-3.5 text-slate-500 transition-transform group-open/details:rotate-180"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
            ...
            {/* Soft divider */}
            <div className="mt-3 h-px bg-gradient-to-r from-transparent via-emerald-400/40 to-transparent" />

            {/* Collapsible content; the outer div controls height, the inner keeps layout clean */}
            <div className="mt-3 grid grid-rows-[0fr] transition-[grid-template-rows] duration-300 ease-out group-open/details:grid-rows-[1fr]">
              <div className="overflow-hidden w-full">
                <div className="max-w-full rounded-lg border border-emerald-500/20 bg-emerald-50/60 dark:bg-white/5 px-3 py-2 text-xs text-emerald-800 dark:text-emerald-200">
                ...
                <div className="mt-4">
                  <a
                    href="#contact"
                    className="inline-flex items-center gap-2 rounded-md border border-emerald-500/30 px-3 py-1.5 text-xs font-semibold
                               text-emerald-700 hover:bg-emerald-50/80
                               dark:text-emerald-200 dark:hover:bg-white/10 transition"
                  >
                    Ask for full scope
                    <svg
...
/* --------- Single Tile (cool, minimal, understandable) --------- */
